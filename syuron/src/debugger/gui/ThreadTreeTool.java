...
public class ThreadTreeTool extends JPanel {
			...
			public void mouseClicked(MouseEvent e) {
				...
				if (selRow != -1) {
					if (e.getClickCount() == 1) {
						ThreadTreeNode node = (ThreadTreeNode) selPath
								.getLastPathComponent();
						// If user clicks on leaf, select it, and issue 'thread'
						// command.
						if (node.isLeaf()) {
	...
	private class ThreadTreeToolListener extends JDIAdapter implements
	...
	class ThreadTreeNode extends DefaultMutableTreeNode {
		...
		private void removeThread(List<String> threadPath,
