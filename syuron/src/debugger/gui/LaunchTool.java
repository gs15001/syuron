...
class LaunchTool {
	...
	private abstract class ArgRep {
		...
		ArgRep(Connector.Argument arg) {
			...
			Border titled = BorderFactory.createTitledBorder(etched,
					arg.description(), TitledBorder.LEFT, TitledBorder.TOP);
	...
	private class BooleanArgRep extends ArgRep {
		...
		@Override
		String getText() {
			return ((Connector.BooleanArgument) arg).stringValueOf(check
					.getModel().isSelected());
		}
	}
	...
	private Connector selectConnector() {
		...
		Map<ButtonModel, Connector> modelToConnector = new HashMap<ButtonModel, Connector>(
				all.size(), 0.5f);
			...
			public void actionPerformed(ActionEvent event) {
				if (radioGroup.getSelection() == null) {
					JOptionPane.showMessageDialog(dialog,
							"Please select a connector type", "No Selection",
	...
	private void configureAndConnect(final Connector connector) {
		final JDialog dialog = new JDialog();
		final Map<String, Connector.Argument> args = connector
				.defaultArguments();
		...
		Border etched = BorderFactory.createEtchedBorder();
		BorderFactory.createTitledBorder(etched, connector.description(),
		...
		JPanel buttonPanel = okCancel(dialog, new ActionListener() {
			...
			public void actionPerformed(ActionEvent event) {
				for (ArgRep ar : argReps) {
					if (!ar.isSpecified()) {
						JOptionPane.showMessageDialog(dialog, ar.arg.label()
				...
				try {
					...
					JOptionPane.showMessageDialog(dialog, "Launch Failure: "
							+ exc, "Launch Failed", JOptionPane.ERROR_MESSAGE);
				}
			}
		});
		content.add(BorderLayout.SOUTH, buttonPanel);
