...
class LaunchTool {
	...
	private abstract class ArgRep {
		...
		ArgRep(Connector.Argument arg) {
			...
			Border titled = BorderFactory.createTitledBorder(etched, arg.description(), TitledBorder.LEFT,
					TitledBorder.TOP);
			...
			public void actionPerformed(ActionEvent event) {
				...
				try {
					if(runtime.explictStart(connector, args)) {
						...
						JOptionPane.showMessageDialog(dialog, "Bad arguments values: See diagnostics window.",
								"Bad arguments", JOptionPane.ERROR_MESSAGE);
					}
				} catch (VMLaunchFailureException exc) {
					JOptionPane.showMessageDialog(dialog, "Launch Failure: " + exc, "Launch Failed",
							JOptionPane.ERROR_MESSAGE);
				}
			}
		});
