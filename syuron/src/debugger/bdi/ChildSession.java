...
class ChildSession extends Session {
	...
	public ChildSession(ExecutionManager runtime, String userVMArgs, String cmdLine, InputListener input,
			OutputListener output, OutputListener error, OutputListener diagnostics) {
	...
	public ChildSession(ExecutionManager runtime, LaunchingConnector connector,
			Map<String, Connector.Argument> arguments, InputListener input, OutputListener output,
	...
	private ChildSession(ExecutionManager runtime, VirtualMachine vm, InputListener input, OutputListener output,
			OutputListener error, OutputListener diagnostics) {
	...
	static private VirtualMachine getVM(OutputListener diagnostics, String userVMArgs, String cmdLine) {
		...
		// とりあえずパラメータ変更
		userVMArgs = userVMArgs.substring(12);
		userVMArgs = " -classpath \"" + userVMArgs + "\"";
		arguments.get("options").setValue(userVMArgs);
		arguments.get("main").setValue(cmdLine);

		// Set<String> ks = arguments.keySet();
		// for(String s : ks) {
		// System.out.println(arguments.get(s));
		// }

		return generalGetVM(diagnostics, connector, arguments);
	}

	static private VirtualMachine generalGetVM(OutputListener diagnostics, LaunchingConnector connector,
	...
	private static class OutputReader extends Thread {
		...
		OutputReader(String threadName, String streamName, BufferedReader stream, OutputListener output,
