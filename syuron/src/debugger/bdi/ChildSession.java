...
class ChildSession extends Session {
	...
	public ChildSession(ExecutionManager runtime, LaunchingConnector connector,
			Map<String, Connector.Argument> arguments, InputListener input,
			OutputListener output, OutputListener error,
	...
	private boolean connectToVMProcess() {
		...
		process = vm.process();
		try {
			in = new PrintWriter(new OutputStreamWriter(
					process.getOutputStream()));
			// ### Note small buffer sizes!
			out = new BufferedReader(new InputStreamReader(
					process.getInputStream(), "SJIS"), 1);
			err = new BufferedReader(new InputStreamReader(
					process.getErrorStream(), "SJIS"), 1);
		} catch (Exception e) {
