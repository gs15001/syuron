...
public abstract class AbstractEventSet extends EventObject implements EventSet {
	...
	/**
	 * Returns the policy used to suspend threads in the target VM for this
	 * event set. This policy is selected from the suspend policies for each
	 * event's request. The one that suspends the most threads is chosen when
	 * the event occurs in the target VM and that policy is returned here. See
	 ...
	 * general contract of the <tt>Collection.toArray</tt> method.
	 *
	 ...
	 *            stored, if it is big enough { return jdiEventSet.XXX(); }
