...
public class EventThread extends Thread {
	...
	public void run() {
		...
		ThreadReference mainTherad = null;
			...
			try {
				...
				while (it.hasNext()) {
					...
					if (event instanceof ClassPrepareEvent) {
						...
						Iterator<Method> i = cpe.referenceType().methods()
								.iterator();
					...
					// System.out.println(mee.thread().name());
					// StepRequest sr = erm.createStepRequest(mee.thread(),
					...
					// if (event instanceof MethodEntryEvent) {
					// mainTherad.suspend();
					// List<StackFrame> list = mainTherad.frames();
					// System.out.println(list.size());
					// for(StackFrame sf : list) {
					...
					// Method m = mee.method();
					// try {
					// List<Location> ml = m.allLineLocations();
					// for (Location l : ml) {
					// BreakpointRequest bpr = erm.createBreakpointRequest(l);
					// bpr.setSuspendPolicy(EventRequest.SUSPEND_ALL);
					// bpr.enable();
					...
					// try {
					// List<LocalVariable> methodList = m.variables();
					// for (LocalVariable lo : methodList) {
					...
					// IntegerValue v = (IntegerValue)mwe.valueToBe();
