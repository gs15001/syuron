/*Copyright (c) Jim Meyering and Paul Eggert
 * Copyright (c) 2009, Stuart D. Gathman.
 ...
 * whitespace" are implemented by modifying the <code>equals</code> and <code>hashcode</code> methods for the objects
 * compared.
 * <p>
 * The basic algorithm is described in: </br> "An O(ND) Difference Algorithm and its Variations", Eugene Myers,
	...
	private change build_script() {
		...
		while (i0 >= 0 || i1 >= 0) {
			if(changed0[i0] || changed1[i1]) {
				...
				// TODO add change when lines are not changed
			}
				...
				if(discards[i] == 2) {
					...
					if(provisional * 4 > length) {
						...
						for (j = 0, consec = 0; j < length; j++) {
							if(discards[i + j] != 2) {
								consec = 0;
							} else if(minimum == ++consec) /* Back up to start of subrun, to cancel it all. */{
								j -= consec;
