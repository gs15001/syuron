...
public class CodeEditorPainter extends JComponent implements TabExpander {
	...
	public void paint(Graphics gfx2) {
		...
		try {
			...
			for (int line = firstInvalid; line <= lastInvalid; line++) {
				...
				int width = paintLine(gfx, tokenMarker, line, x) - x + 5 /* Yay */;
				if(valid) {
					if(tokenMarker != null) {
	...
	public final void invalidateLineRange(int firstLine, int lastLine) {
		repaint(0, editor.lineToY(firstLine) + fm.getMaxDescent() + fm.getLeading(), getWidth(),
	...
	protected void paintLineHighlight(Graphics gfx, int line, int y) {
		...
		if(selectionStart == selectionEnd) {
			...
			if(editor.isSelectionRectangular()) {
				...
				x1 = editor._offsetToX(line, Math.min(lineLen, selectionStart
						- editor.getLineStartOffset(selectionStartLine)));
				x2 = editor._offsetToX(line, Math.min(lineLen, selectionEnd
						- editor.getLineStartOffset(selectionEndLine)));
				if(x1 == x2) {
					x2++;
				}
			} else if(selectionStartLine == selectionEndLine) {
				x1 = editor._offsetToX(line, selectionStart - lineStart);
				x2 = editor._offsetToX(line, selectionEnd - lineStart);
